...
const Top = ({ value, dataID }) => {
    ...
    $(node.current).resizable({
      ...
      stop: (event, ui) => {
        ...
        setTimeout(() => {
          ...
          if (px !== oldPX) {
            node.current.parentNode.style.paddingTop = null;
      ...
      resize: (event, ui) => {
        setTimeout(() => {
          node.current.parentNode.style.paddingTop = `${ui.size.height > 0 ? ui.size.height : 0}px`;
  ...
  return (
    <div ref={node} className="editor__resizebox-top" style={{ height: `${val}`, bottom: `-${val}` }}>
