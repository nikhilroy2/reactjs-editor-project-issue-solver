  ...
  $.widget('ui.rotatable', $.ui.mouse, {
    ...
    */
    ...
    rotateElement(event) {
      ...
      let rotateAngle = this._calculateRotateAngle(event)
      const previousRotateAngle = this.elementCurrentAngle
      ...
      if (this._trigger('rotate', event, ui) === false) {
        this.elementCurrentAngle = previousRotateAngle
        return false
      } if (ui.angle.current !== rotateAngle) {
    ...
    wheelRotate(event) {
      let angle = this._angleInRadians(Math.round(event.originalEvent.deltaY / 10))
    ...
    _calculateElementCenter() {
      ...
      if (this.element.css('transform-origin') !== undefined) {
        const originPx = this.element.css('transform-origin').match(/([\d.]+)px +([\d.]+)px/)
        if (originPx != null) {
          return {
    ...
    _calculateSnap(radians) {
      let degrees = this._angleInDegrees(radians)
    ...
    _calculateRotateAngle(event) {
      ...
      let rotateAngle = mouseAngle - this.mouseStartAngle + this.elementStartAngle
    ...
    _create() {
      ...
      handle.draggable({ helper: 'clone', start: this._dragStart, handle })
    ...
    _getTransforms(angle) {
      let transforms = `rotate(${angle}rad)`
    ...
    _performRotation(angle) {
      ...
      const transforms = this._getTransforms(angle)
